...
func InternalServerError(session *engine.Session, err error, request *http.Request, response http.ResponseWriter) {

	err_message := err.Error()

	if err_message != "" {
		session.Console.Error(fmt.Sprintf("> %s %s %d: \"%s\"", request.Method, request.URL.Path, http.StatusInternalServerError, err_message))
	} else {
